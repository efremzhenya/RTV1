...
/*   Updated: 2021/05/13 23:55:14 by mellie           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
float	intersect_cone(t_vec3 cam_origin, t_vec3 ray_dir, t_object *cone)
{
	...
	k = tan(data->angle / 2.0 * (M_PI / 180.0));
	k = k * k + 1;
	quadric.a = vec3_dot_product(ray_dir, ray_dir) - k * pow(vec3_dot_product(ray_dir, cone_dir), 2);
	quadric.b = 2 * vec3_dot_product(ray_dir, cone_cam) - 2 * k * vec3_dot_product(ray_dir, cone_dir) * vec3_dot_product(cone_cam, cone_dir);
	quadric.c = vec3_dot_product(cone_cam, cone_cam) - k * pow(vec3_dot_product(cone_cam, cone_dir), 2);
